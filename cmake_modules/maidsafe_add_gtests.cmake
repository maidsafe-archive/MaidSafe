  ...
  if((${GTEST_NAME} MATCHES ${MAIDSAFE_TEST_TYPE}.+) OR (${MAIDSAFE_TEST_TYPE} MATCHES "ALL"))
    ...
    if(THIS_GTEST_EXCLUDED EQUAL -1)
      if("${GTEST_NAME}" MATCHES "^DISABLED_")
        ...
        set(CATCH_EXCEPTIONS "--gtest_catch_exceptions=0")
        if(WIN32)
          if(${CI_BUILD_ENVIRONMENT})
            set(CATCH_EXCEPTIONS "--gtest_catch_exceptions=1")
          endif()
        endif()
        add_test(NAME ${FULL_GTEST_NAME} COMMAND ${CMAKE_SOURCE_DIR}/tools/run_test.py $<TARGET_FILE:${TEST_EXECUTABLE}> ${FULL_GTEST_NAME} ${CATCH_EXCEPTIONS} ${CMAKE_SOURCE_DIR})
      endif()
      if("${GTEST_NAME}" MATCHES "^FUNC_" OR "${GTEST_NAME}" MATCHES "^DISABLED_FUNC_")
        set_property(TEST ${FULL_GTEST_NAME} PROPERTY LABELS ${CamelCaseProjectName} ${PROJECT_NAME} Functional ${GTEST_FIXTURE_NAME})
      ...
      elseif("${GTEST_NAME}" MATCHES "^BEH_" OR "${GTEST_NAME}" MATCHES "^DISABLED_BEH_")
        set_property(TEST ${FULL_GTEST_NAME} PROPERTY LABELS ${CamelCaseProjectName} ${PROJECT_NAME} Behavioural ${GTEST_FIXTURE_NAME})
