...
#   endif()
  ...
  foreach(GtestSourceFile ${GtestSourceFiles})
    get_gtest_typedef_types(${GtestSourceFile})
  endforeach()

  foreach(GtestSourceFile ${GtestSourceFiles})
    get_gtest_fixtures_types(${GtestSourceFile})
    get_gtest_fixtures_value_parameters(${GtestSourceFile})
    get_gtest_fixtures_type_parameters(${GtestSourceFile})
  endforeach()

  foreach(GtestSourceFile ${GtestSourceFiles})
    add_gtest_registered_typed_tests(${GtestSourceFile} ${TestTarget})
    add_gtest_non_type_parameterised(${GtestSourceFile} ${TestTarget})
  endforeach()
  set(AllGtests ${AllGtests} PARENT_SCOPE)
endfunction()
...
function(get_gtest_typedef_types GtestSourceFile)
  file(STRINGS ${GtestSourceFile} FileContents NEWLINE_CONSUME)
...
function(get_gtest_fixtures_types GtestSourceFile)
  file(STRINGS ${GtestSourceFile} FileContents NEWLINE_CONSUME)
...
function(get_gtest_fixtures_value_parameters GtestSourceFile)
  file(STRINGS ${GtestSourceFile} FileContents NEWLINE_CONSUME)
...
function(get_gtest_fixtures_type_parameters GtestSourceFile)
  file(STRINGS ${GtestSourceFile} FileContents NEWLINE_CONSUME)
...
# test name.
function(add_gtest_registered_typed_tests GtestSourceFile TestTarget)
  ...
  ms_remove_block_comments(FileContents)
...
function(add_gtest_non_type_parameterised GtestSourceFile TestTarget)
  set(TestExecutable ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${TestTarget}${TestPostfix})
  file(STRINGS ${GtestSourceFile} GtestNames REGEX "^(TYPED_)?TEST(_[FP])?\\(")
  foreach(GtestName ${GtestNames})
...
function(add_maidsafe_test GtestFixtureName GtestName FullGtestName TestExecutable)
  if((${GtestName} MATCHES "${MAIDSAFE_TEST_TYPE}.+") OR (${MAIDSAFE_TEST_TYPE} MATCHES "ALL"))
    ...
    if((NOT ThisGtestExcluded EQUAL -1) OR "${GtestName}" MATCHES "^DISABLED_")
      add_test(${FullGtestName} "__disabled__")
    else()
  ...
  if("${GtestName}" MATCHES "^FUNC_" OR "${GtestName}" MATCHES "^DISABLED_FUNC_")
    set_property(TEST ${FullGtestName} PROPERTY LABELS ${CamelCaseProjectName} Functional ${NetworkTestLabel})
  ...
  elseif("${GtestName}" MATCHES "^BEH_" OR "${GtestName}" MATCHES "^DISABLED_BEH_")
    set_property(TEST ${FullGtestName} PROPERTY LABELS ${CamelCaseProjectName} Behavioural ${NetworkTestLabel})
...
#     set_property(TEST ${FullGtestName} PROPERTY LABELS ${CamelCaseProjectName} Private ${NetworkTestLabel})
