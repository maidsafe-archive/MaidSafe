...
foreach(MetaFile ${MetaFiles})
  ...
  foreach(MessageType ${AllTypes})
    ...
    if(MessageType)
      ...
      string(REGEX MATCH "[Gg][Ee][Tt][Cc][Aa][Cc][Hh][Ee][Aa][Bb][Ll][Ee]" IsGetCacheable "${MessageType}")
      string(REGEX MATCH "[Pp][Uu][Tt][Cc][Aa][Cc][Hh][Ee][Aa][Bb][Ll][Ee]" IsPutCacheable "${MessageType}")
      ...
      if(IsGetCacheable)
        list(APPEND GetCacheableMessages "${Action}:${SourcePersona}:${DestinationPersona}")
        set(HaveCacheable TRUE)
      endif()
      if(IsPutCacheable)
        list(APPEND PutCacheableMessages "${Action}:${SourcePersona}:${DestinationPersona}")
        set(HaveCacheable TRUE)
      endif()
...
if(HaveCacheable)
  ...
  list(APPEND Variants "//==================================================================================================\n\n")
  list(APPEND Variants "template<typename CacheableVariant>\n")
  list(APPEND Variants "inline bool GetVariantOfCacheable(const maidsafe::nfs::TypeErasedMessageWrapper& message,\n")
  list(APPEND Variants "                                  CacheableVariant& cacheable_variant);\n\n")

  foreach(CacheType GetCacheable PutCacheable)
    unset(Actions)
    foreach(MessageType ${${CacheType}Messages})
    ...
    list(APPEND Variant "// Modify in <sub-project>/cmake/*.message_types.meta if required, not here.\n")
    list(APPEND Variant "typedef boost::variant<\n    ${AllMessageTypedefs}> ${CacheType}Messages;\n\n")

    list(APPEND Variant "// Auto-generated function.\n")
    list(APPEND Variant "// Modify in <sub-project>/cmake/*.message_types.meta if required, not here.\n")
    list(APPEND Variant "template<>\n")
    list(APPEND Variant "inline bool GetVariantOfCacheable<${CacheType}Messages>(\n")
    list(APPEND Variant "    const maidsafe::nfs::TypeErasedMessageWrapper& message,\n")
    list(APPEND Variant "    ${Spaces}${CacheType}Messages& cacheable_variant) {\n")
    list(APPEND Variant "  auto action(std::get<0>(message));\n")
    list(APPEND Variant "  auto source_persona(std::get<1>(message).data);\n")
    list(APPEND Variant "  auto destination_persona(std::get<2>(message).data);\n")
    list(APPEND Variant "  switch (action) {\n")
    foreach(Action ${Actions})
      ...
      foreach(SourcePersona ${${Action}Sources})
        ...
        foreach(DestinationPersona ${${Action}${SourcePersona}})
          ...
          list(APPEND Variant "              cacheable_variant = ${CacheType}Messages(\n")
          list(APPEND Variant "                  ${Action}From${SourcePersona}To${DestinationPersona}(message));\n")
          list(APPEND Variant "              return true;\n")
        endforeach()
        list(APPEND Variant "            default:\n")
